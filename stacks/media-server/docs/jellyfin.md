...
Nothing. Jellyfin keeps its configuration in XML files under `/config`; the expected state
is below. `encoding.xml` is the authoritative record of the transcoding settings.
