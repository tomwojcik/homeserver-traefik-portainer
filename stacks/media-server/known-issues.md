...
10. DSM updates rewrite `/usr/syno/share/nginx/*.mustache`, undoing the port change from the root README (DSM back on 80/443). DSM's nginx then grabs 443 the next time Traefik restarts, and every hostname redirects to `:5001` with the DSM login (seen 2026-09-14). Check `sudo netstat -tlnp | grep -E ':(80|443) '`: it must show docker-proxy, not nginx. Fix: re-run the README's `sed` on the three mustache files, `sudo systemctl restart nginx`, `docker restart traefik`. Re-check after every DSM update.
