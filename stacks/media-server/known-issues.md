...
8. Synology's Docker proxies LAN connections through the host, so Traefik sees every LAN client as the bridge gateway (`172.22.0.1` on this host; check the first column of `docker logs traefik`). `LAN_CIDR` must therefore include that address as a `/32` (verified 2026-09-14: `192.168.0.0/16,10.0.0.0/8,172.22.0.1/32`). Containers hairpinning through Traefik keep their own `172.22.0.x` addresses and stay blocked. Consequence: the gate cannot distinguish LAN from a future port-forward or tunnel (both would also arrive as the gateway); nothing is exposed today, so its remaining job is blocking app-to-app traffic via the public hostnames.
